...
namespace ConsoleTest
{
    class Program
    {
        ...
        public static void WelcomeScreen()
        {
            ...
            Controller.contents.Add(new Content() { action = Controller.ShowBest, lines = new List<List<string>>() { new List<string>() { "", "推荐装备", "现算法不考虑天赋/具名/套装影响，装备额外考虑词条颜色统一", $"半数以上词条优质(>={Math.Round(double.Parse(Config.GetValueConf("bestFilterThreshold")) * 100, 0)}%)则进入推荐，可配置文件自行更改", "" } } });
